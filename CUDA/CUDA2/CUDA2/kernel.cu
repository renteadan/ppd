...
////////////////////////////////////////////////////////////////////////////////
// Constants
const float angle = 1.0f;  // angle to rotate image by (in radians)

float tx = 0.5f, ty = 0.5f;  // image translation
float scale = 1.0f / 512.0f;   // image scale
...
__global__ void d_render(float *d_output, int width, int height, float tx,
...
void runTest(int argc, char **argv) {
  ...
  // transformKernel<<<dimGrid, dimBlock, 0>>>(dData, width, height, angle,
  // tex);
  d_render<<<dimGrid, dimBlock>>>(dData, width, height, tx, ty, scale, cx,
                                  cy, tex);
